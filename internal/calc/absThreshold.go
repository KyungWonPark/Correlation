...
func absThreshold(inputMat *mat64.Dense, outputMat *mat64.Dense, thr float64, sub float64, order <-chan int, wg *sync.WaitGroup) {
	...
	for {
		...
		if ok {
			for t := 0; t < inputCols; t++ {
				value := math.Abs(inputMat.At(index, t))
				if thr >= value {
					value = sub
				}
...
// AbsThreshold does absolute thresholding
func (p *PipeLine) AbsThreshold(inputMat *mat64.Dense, outputMat *mat64.Dense, thr float64, sub float64) {
	...
	for i := 0; i < p.numPoper; i++ {
		go absThreshold(inputMat, outputMat, thr, sub, order, &wg)
	}
