    ...
    for i in range(len(clusters)):
        ...
        for idx0 in range(n):
            for idx1 in range(0, idx0 + 1):
                ...
                accPearson = accPearson + sim[idxFrom][idxTo]
...
sim = loadCSV(RESULTDIR + "sim.csv")
greyList = loadCSV(RESULTDIR + "greyList.txt")

# ANC and WCC Calculation
...
x = [
    0.000000,0.050000,0.100000,0.150000,0.200000,0.250000,0.300000,0.350000,0.400000,0.450000,0.500000,0.550000,
    0.600000,0.650000,0.700000,0.750000,0.800000,0.850000,0.900000,0.950000,0.960000,0.970000,0.980000,0.990000,
]
...
for thr in thrs:
    ...
    clusters = getClusters(eigVal, eigVec)
    
    wcc = getWCC(clusters, sim)
...
plt.plot(x, WCCs)
plt.title("WCC by Threshold Value", fontsize=20)
plt.xlabel('Threshold value', fontsize=18)
plt.xticks(np.arange(0.0, 1.05, 0.05))
plt.ylabel('WCC', fontsize=18)
plt.savefig(OUTPUTDIR + "wcc.png", dpi=300)