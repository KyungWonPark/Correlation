...
func main() { // thrStart thrEnd thrItv timeStart timeEnd postFix
	...
	if _, err := os.Stat(RESULTDIR); os.IsNotExist(err) {
		...
		log.Fatalf("Such directory does not exist!!")
	}
	...
	go func() {
		for _, file := range fileList {
			dest := pl.Malloc()
			doSampling(DATADIR+"/fMRI-Smoothed/"+file, timeStart, timeEnd, ringBuffer[dest], pl.GetNP())
			pl.Push(dest)
		}
...

	{
		...
		{
			temp0 := mat64.NewDense(13362, timePeriod, nil)
			temp1 := mat64.NewDense(13362, timePeriod, nil)
