...
func sampling(img *nifti.Nifti1Image, timeStart int, order <-chan int, wg *sync.WaitGroup, timeSeries *mat64.Dense) {
	for {
		timePoint, ok := <-order
		if ok {
			for i, vox := range greyVoxels {
				seed := Voxel{2*vox.x - 1, 2*vox.y - 1, 1 + 2*vox.z - 1}
				timeSeries.Set(i, timePoint-timeStart, convolution(img, timePoint, seed))
			}
			wg.Done()
		} else {
			break
...
func doSampling(path string, timeStart int, timeEnd int, timeSeries *mat64.Dense, numLoader int) {
	...
	for i := 0; i < numLoader; i++ {
		go sampling(&img, timeStart, order, &wg, timeSeries)
	}

	for timePoint := timeStart; timePoint < timeEnd; timePoint++ {
		order <- timePoint
	}
	wg.Wait()
