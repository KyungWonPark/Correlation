...
func main() { // thrStart thrEnd thrItv isDebugMode
	...
	matBufferShm, err := shm.Create(1000 * 1000 * 8)
	if err != nil {
		log.Fatalf("Failed to create shared memory region: %s\n", err)
	}

	eigValShm, err := shm.Create(1000 * 1 * 8)
	if err != nil {
		log.Fatalf("Failed to create shared memory region: %s\n", err)
	}
	...
	arrEigVal := make([]float64, 1000*1)
	eigVal := mat64.NewDense(1000, 1, arrEigVal)
	arrEigVec := make([]float64, 1000*1000)
	eigVec := mat64.NewDense(1000, 1000, arrEigVec)

	testMat := mat64.NewDense(1000, 1000, nil)
	...
	for thr = thrStart; thr < thrEnd; thr += thrItv {
		...
		getOnly1k(thredMat, testMat)

		pl.Laplacian(testMat) // Now thredMat is a Laplacian matrix

		mat64tocArr(testMat, pMatBuffer) // Copy thresholded matrix to MAGMA matrix buffer
		...
		if isDebugMode {
			...
			eigValMat := mat64.NewDense(1000, 1000, nil)
			for i := 0; i < 1000; i++ {
				eigValMat.Set(i, i, eigVal.At(i, 0))
			}

			// U^T * A
			result0 := mat64.NewDense(1000, 1000, nil)
			result0.Mul(eigVec, testMat)

			// S * U^T
			result1 := mat64.NewDense(1000, 1000, nil)
			...
			if !isSame {
				...
				diff := mat64.NewDense(1000, 1000, nil)
