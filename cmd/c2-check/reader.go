	...
	for {
		...
		if ok {
			for i, vox := range greyVoxels {
				...
				timeSeries.Set(i, timePoint, convolution(img, timePoint, seed))
			}
			wg.Done()
		} else {
			break
...
func doSampling(path string) *mat64.Dense {
	var img nifti.Nifti1Image
	img.LoadImage(path, true)

	numLoader := runtime.NumCPU()
	...
	fmt.Printf("%s file Dimensions: \n", path)
	fmt.Printf("X: %d Y: %d Z: %d T: %d\n", xLength, yLength, zLength, tLength)

	timeSeries := mat64.NewDense(13362, tLength, nil)
	...
	for timePoint := 0; timePoint < tLength; timePoint++ {
		order <- timePoint
	}
	wg.Wait()
